...
export default function Dashboard() {
  ...
  const analyzeCoin = (coin: CoinData): AnalyzedCoin => {
    ...
    // 진입가 = 현재가 (항상)
    // 손절 = -3% 고정, 목표 = 시그널별 차등
    let entry_price = price // 항상 현재가 = 진입가
    let target_price, stop_loss, risk_reward

    if (signal === 'strong_buy') {
      // 강력 매수: 목표 +4.5%, 손절 -3%, 손익비 1:1.5
      target_price = price * 1.045
      stop_loss = price * 0.97
      risk_reward = '1:1.5'
    } else if (signal === 'buy') {
      // 매수: 목표 +4.2%, 손절 -3%, 손익비 1:1.4
      target_price = price * 1.042
      stop_loss = price * 0.97
      risk_reward = '1:1.4'
    } else if (signal === 'hold') {
      // 관망: 목표 +3.6%, 손절 -3%, 손익비 1:1.2
      target_price = price * 1.036
      stop_loss = price * 0.97
      risk_reward = '1:1.2'
    } else {
      // 매도/숏: 목표 +3%, 손절 -3%, 손익비 1:1
